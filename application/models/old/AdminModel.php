...
public function deletemoto($id){
    $this->db->delete('aquadeposit', array('id' => $id)); 
}
public function deletemotodin($id){
    $this->db->delete('aquacredit', array('id' => $id)); 
}
...
public function get4mostPackage(){
...
public function count4mostPackage(){
    $this->db->select("COUNT(*) as count_id");
    $this->db->group_by('package_id');
    $q = $this->db->get('tbl_transaction');
	return $q->result();
}
public function getratePackage(){
    $this->db->order_by("ratings", "desc");
...
public function insertSales($user){
    $this->db->insert('tbl_deposit', $user);
}
public function insertAquaSales($user){
    $this->db->insert('aquadeposit', $user);
}
public function getAllAquaPayment(){
    $q = $this->db->get('aquadeposit');
    return $q->result();
}


public function countTransactione(){
    $q = $this->db->get_where('aquadeposit');
    return $q->result();
}


public function insertAquaCredit($user){
    $this->db->insert('aquacredit', $user);
}
public function getAllAquaCredit(){
    $q = $this->db->get('aquacredit');
    return $q->result();
}
